...
# --- 1. KONFIGURACIJA ---
st.set_page_config(page_title="Rakija Master Pro", page_icon="🥃", layout="centered", initial_sidebar_state="collapsed")
...
st.markdown(f"""
    <style>
    /* =========================================
       TOTALNO UNIŠTAVANJE STREAMLIT IKONICA
       ========================================= */
    
    /* 1. Gornji meni (Deploy, GitHub, Menu) */
    header, [data-testid="stHeader"], [data-testid="stToolbar"], .stDeployButton, [data-testid="stAppDeployButton"] {{
    ...
    /* 2. Donji desni ugao (Manage App, Running status) */
    footer,[data-testid="stFooter"], [data-testid="stStatusWidget"], [class*="viewerBadge"] {{
        display: none !important;
        visibility: hidden !important;
        opacity: 0 !important;
        pointer-events: none !important;
    ...
    /* 3. Svi ifrejmovi (Streamlit Cloud ubacuje Manage App kao iframe) */
    iframe {{
        display: none !important;
        visibility: hidden !important;
        opacity: 0 !important;
    ...
    /* 4. Moderni CSS :has() selektor - pronalazi kontejner koji drzi Streamlit link i brise ga */
    div:has(> a[href*="streamlit"]), div:has(> a[href*="manage"]) {{
        display: none !important;
        visibility: hidden !important;
        opacity: 0 !important;
        pointer-events: none !important;
    ...
    /* 5. Sakrivanje bilo kog elementa koji je fiksiran dole desno (poslednja linija odbrane) */
...
# ==========================================
    ...
    def naslov_alata(tekst, podnaslov=""):
        st.markdown(f"<h2 style='color:{GOLD}; font-size:24px; font-weight:bold; margin-bottom: 0px;'>{tekst}</h2>", unsafe_allow_html=True)
        if podnaslov:
